...
internal sealed class GeminiTaskRunner
{
    ...
    public async Task<GeminiTaskRunResult> RunAsync(ScheduledGeminiTask task)
    {
        ...
        int timeoutMinutes = Math.Clamp(settings.TaskCompletionTimeoutMinutes, 1, 240);
        TimeSpan approvalWindow = TimeSpan.FromMinutes(timeoutMinutes + 5);
        ...
        try
        {
            ...
            GeminiTaskCompletionResult completion = await sidePanel.WaitForTaskCompletionAsync(
